...
for await (const entry of Deno.readDir(base)) {
    ...
    //         return `https://raw.githack.com/u2ui/${entry.name}/main/tests/${test.name}  `;
    //     }).join('\n');
...
function installPart(entry, path) {
    ...
    if (name.endsWith('.el')) {
        html = dedent(`
        <link href="../../../${name}@${version}/${blankName}.min.css" rel=stylesheet>
        <script src="../../../${name}@${version}/${blankName}.min.js" type=module></script>`);
    }
    if (name.endsWith('.class')) {
        html = `<link href="../../../${name}@${version}/${blankName}.min.css" rel=stylesheet>`;
    }
    if (name.endsWith('.attr')) {
        html = `<script src="../../../${name}@${version}/${blankName}.min.js" type=module></script>`;
    }
    if (name.endsWith('.js')) {
        const js = `import * as module from "../../../${name}@${version}/${blankName}.min.js"`;
        return '```js\n'+js+'\n```';
    }
    return html ? '```html\n'+html+'\n```' : null;
}
...
async function demoPart(entry){
    try {
        ...
        return tests.map(test=>{
            ...
            return `[${test.name}](http://gcdn.li/u2ui/${entry.name}@main/tests/${test.name})  `
