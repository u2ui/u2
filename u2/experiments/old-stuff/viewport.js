...
const onScroll = function(){
    ...
    docEl.classList[upScroll?'add':'remove']('u2-scroll-up');
    lastY = scrollY;
    const isTop = lastY <= 20; // 0 is not enough on iOS
    docEl.classList[isTop?'remove':'add']('u2-scroll-not-top');
};
onScroll();
...
function check(){
    ...
    document.documentElement.style.setProperty('--u2-vh-min', hMin +'px');
    lastHMin = hMin;
};
check();
