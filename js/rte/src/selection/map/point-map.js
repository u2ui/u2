...
export class PointMap {
    ...
    insertText(node, offset, data) {
        ...
        if (typeof data !== 'string') throw new TypeError('Inserted text must be a string');
        node.insertData(offset, data);
        for (const point of this.#points.values()) {
            if (point.node !== node) continue;
            if (point.offset > offset || point.offset === offset && point.affinity === 'forward') point.offset += data.length;
        }
        return node;
    }
