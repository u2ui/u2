...
`input-pipeline.js` installs this behavior for one `Surface`. Installation is
explicit and reversible: construct one pipeline for a surface and call
`dispose()` during module cleanup. `[Symbol.dispose]()` exposes it to `using`.
Disconnecting the surface also disposes its pipeline automatically.
...
- With a command registry, a cancelable `beforeinput` whose `inputType` has a
  ...
  target range unchanged. Uncancelable events, plain-text hosts, and active
  composition are never routed.
