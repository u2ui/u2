...
test('point map: text insertion maps offsets and equal-boundary affinity', () => withFixture(
    '<div>abcd</div>', root => {
        const text = root.firstElementChild.firstChild;
        const backward = new Point(text, 2, 'backward');
        const forward = new Point(text, 2, 'forward');
        const after = new Point(text, 4);
        const map = new PointMap([backward, forward, after]);
        same(map.insertText(text, 2, 'ä🙂'), text);
        equal(text.data, 'abä🙂cd');
        equal(map.get(backward).offset, 2);
        equal(map.get(forward).offset, 5);
        equal(map.get(after).offset, 7);
        throws(() => map.insertText(root.firstElementChild, 0, 'x'), TypeError);
        throws(() => map.insertText(text, 20, 'x'), RangeError);
        throws(() => map.insertText(text, 0, null), TypeError);
    }
));
